...
import { writeRunRecord, runRecordPath } from "../src/process/records.js";
...
describe("almanac capture sweep", () => {
  ...
  it("does not discover Almanac-generated operation transcripts", async () => {
    await withTempHome(async (home) => {
      const repo = await makeRepo(home, "repo");
      await scaffoldWiki(repo);
      const old = new Date("2026-05-11T10:00:00.000Z");
      const codexDir = join(home, ".codex", "sessions");
      await mkdir(codexDir, { recursive: true });
      const internal = join(codexDir, "internal.jsonl");
      await writeFile(
        internal,
        `${JSON.stringify({
          type: "session_meta",
          payload: { id: "internal", cwd: repo },
        })}\n` +
          `${JSON.stringify({
            type: "response_item",
            payload: {
              type: "message",
              content:
                "Runtime context:\n- Wiki pages directory: /repo/.almanac/pages\n\nCommand context:\n- Command: capture",
            },
          })}\n`,
        "utf8",
      );
      await utimes(internal, old, old);

      const external = join(codexDir, "external.jsonl");
      await writeFile(
        external,
        `${JSON.stringify({
          type: "session_meta",
