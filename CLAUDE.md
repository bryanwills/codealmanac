...
| `src/services/setup/` | Setup and uninstall product workflows over agent readiness/config state, provider fix commands, instruction targets, automation cleanup, and guide cleanup while CLI setup files own terminal UI | `agent-choice.ts`, `provider-fix-command.ts`, `auto-commit.ts`, `instructions.ts`, `uninstall.ts`, `index.ts` |
