...
| `src/services/wiki/` | Wiki product workflows that coordinate stores/indexes/filesystem mechanics | `search.ts`, `page-view.ts`, `health.ts`, `registry.ts`, `autoregistration.ts`, `topic-read.ts`, `topic-mutations.ts`, `topic-page-rewrite.ts`, `page-topic-mutations.ts`, `reviews.ts` |
