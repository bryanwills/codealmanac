...
| `src/edges/cli/` | Commander registration, process argv handling, sqlite-free recovery path, CLI-edge event rendering, terminal setup/uninstall UI, and local CLI process lifetimes | `register-commands.ts`, `register-query-commands.ts`, `register-edit-commands.ts`, `register-review-commands.ts`, `register-page-topic-commands.ts`, `register-migrate-commands.ts`, `register-topics-commands.ts`, `register-lifecycle-run-commands.ts`, `register-sync-commands.ts`, `register-jobs-commands.ts`, `register-automation-commands.ts`, `register-setup-commands.ts`, `setup/`, `uninstall.ts`, `uninstall-render.ts`, `serve.ts` |
| `src/edges/cli/setup/` | Interactive setup terminal UI, prompts, setup step rendering, and setup edge orchestration over setup services | `index.ts`, `types.ts`, `input.ts`, `output.ts`, `setup-plan.ts`, `agent-choice.ts`, `instruction-target-choice.ts` |
| `src/edges/viewer/` | Local HTTP viewer edge, static asset serving, route handling, and viewer-only read models | `server.ts`, `static.ts`, `read-model/` |
| `src/edges/viewer/read-model/` | Viewer-specific API DTO assembly over wiki stores, review store, and job projections | `api.ts`, `global-api.ts`, `jobs.ts`, `jobs-api.ts`, `review-api.ts`, `job-types.ts` |
...
| `src/platform/` | Local machine integration for install, self-update, launchd/path/job mechanics, diagnostics probes, local transcript discovery/snapshot reads, external source mechanics, and child-process spawning | `install/`, `update/`, `automation/`, `diagnostics/`, `transcripts/`, `github/`, `jobs/` |
| `src/agent/` | Agent provider enablement, readiness, auth, instructions, and concrete runtime support | `provider-enablement.ts`, `readiness/`, `auth/`, `runtime/` |
| `src/agent/runtime/` | Concrete provider execution contracts and provider runtime adapters over shared agent-runtime facts | `types.ts`, `job-runner.ts`, `providers/claude.ts`, `providers/claude/`, `providers/codex.ts`, `providers/codex/` |
...
| `src/shared/` | Small cross-cutting helpers and neutral contracts with no product workflow ownership | `duration.ts`, `agent-provider.ts`, `agent-runtime/`, `diagnostics.ts`, `operation-spec.ts`, `pid-liveness.ts`, `transcripts.ts`, `worker-program.ts` |
...
| `src/services/diagnostics/` | Install, agent, update, and wiki diagnostic read models for `almanac doctor`; platform probe facts come through `src/shared/diagnostics.ts` | `doctor.ts`, `install.ts`, `agents.ts`, `updates.ts`, `types.ts`, `index.ts` |
...
| `src/services/jobs/runtime/` | Job execution lifecycle over persisted records/specs/logs and agent runtime execution | `start.ts`, `background-start.ts`, `queue-drain.ts`, `executor.ts`, `events.ts`, `finalization.ts`, `wiki-effects.ts` |
| `src/services/jobs/projections/` | Derived job read models for viewer titles, transcript source labels, agent traces, warnings, and parsed log events | `view.ts`, `log-events.ts`, `agent-traces.ts`, `warnings.ts`, `types.ts` |
| `src/services/lifecycle/` | Lifecycle product workflows for init, Absorb, Garden, and prepared operation handoff | `workflows.ts`, `operation-results.ts`, `operations/`, `absorb/`, `index.ts` |
| `src/services/lifecycle/operations/` | Provider-neutral Build/Absorb/Garden operation spec and run construction | `run.ts`, `build.ts`, `absorb.ts`, `garden.ts`, `spec.ts`, `types.ts`, `reports.ts` |
...
| `src/services/setup/` | Setup and uninstall product workflows over agent readiness/config state, provider fix commands, global install state/execution, instruction targets, automation cleanup, and guide cleanup while CLI edge files own terminal UI | `agent-choice.ts`, `provider-fix-command.ts`, `global-install.ts`, `auto-commit.ts`, `instructions.ts`, `uninstall.ts`, `index.ts` |
...
| `src/stores/jobs/` | Job record, spec, log, lock, and persisted job/log schema mechanics under `.almanac/jobs/` | `index.ts`, `records.ts`, `specs.ts`, `logs.ts`, `worker-lock.ts`, `types.ts`, `record-schema.ts`, `log-entry.ts` |
| `src/stores/wiki-files/` | Mechanical `.almanac/` scaffolding, repo file writes, page-file counts, and absorb-log scans used by wiki initialization, setup/build state, and doctor checks | `scaffold.ts`, `pages.ts`, `absorb-logs.ts` |
