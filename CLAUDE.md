...
| `src/edges/cli/` | Commander registration, process argv handling, sqlite-free recovery path, CLI-edge event rendering, and local CLI process lifetimes | `register-commands.ts`, `register-query-commands.ts`, `register-edit-commands.ts`, `register-review-commands.ts`, `register-page-topic-commands.ts`, `register-migrate-commands.ts`, `register-topics-commands.ts`, `register-lifecycle-run-commands.ts`, `register-sync-commands.ts`, `register-jobs-commands.ts`, `register-automation-commands.ts`, `register-setup-commands.ts`, `serve.ts` |
...
| `src/platform/` | Local machine integration for install, self-update, launchd/path/job mechanics, diagnostics probes and probe result contracts, local transcript discovery/snapshot reads, external source mechanics, and child-process spawning | `install/`, `update/`, `automation/`, `diagnostics/`, `transcripts/`, `github/`, `jobs/` |
...
| `src/services/diagnostics/` | Install, agent, update, and wiki diagnostic read models for `almanac doctor`; platform probe facts are imported from `src/platform/diagnostics/types.ts` | `doctor.ts`, `install.ts`, `agents.ts`, `updates.ts`, `types.ts`, `index.ts` |
...
| `src/services/setup/` | Setup and uninstall product workflows over agent readiness/config state, provider fix commands, global install state/execution, instruction targets, automation cleanup, and guide cleanup while CLI setup files own terminal UI | `agent-choice.ts`, `provider-fix-command.ts`, `global-install.ts`, `auto-commit.ts`, `instructions.ts`, `uninstall.ts`, `index.ts` |
...
| `src/stores/wiki-files/` | Mechanical `.almanac/` scaffolding, repo file writes, and page-file counts used by wiki initialization and setup/build state | `scaffold.ts`, `pages.ts` |
