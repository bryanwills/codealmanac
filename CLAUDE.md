...
| `src/services/setup/` | Setup and uninstall product workflows over agent readiness/config state, automation cleanup, and guide cleanup while CLI setup files own terminal UI | `agent-choice.ts`, `auto-commit.ts`, `uninstall.ts`, `index.ts` |
