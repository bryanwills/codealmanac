...
The local GitHub MVP uses the GitHub CLI before adding Octokit, Composio, or a hosted GitHub App. The implemented command surface accepts `almanac ingest github:pr:123`, `almanac ingest github:issue:11`, GitHub pull-request URLs, GitHub issue URLs, and generic HTTP(S) URLs. Known GitHub URLs resolve into PR or issue source briefs; unknown HTTP(S) URLs resolve into `web.url` sources and are still handed to Absorb as source material. Malformed explicit GitHub source refs still fail early. [@github-source-ref]

The resolver infers the owner and repository from `git remote get-url origin` for shorthand GitHub refs, accepts GitHub HTTPS and SSH remotes, and uses repository identity embedded in GitHub URLs when present. GitHub PR and issue sources check `gh --version` and `gh auth status` before producing a small source fact object with kind, raw ref, `owner/repo`, canonical URL, and source number. Setup failures are ordinary `Error` subclasses with actionable fix text for installing or authenticating `gh`. Generic web URL sources do not check `gh`. [@github-source-resolver] [@github-source-tests]
