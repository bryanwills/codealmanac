...
`PageView` is defined in `src/wiki/query/page-view.ts` and includes: slug, title, description, file\_path, updated\_at, archived\_at, superseded\_by, supersedes, topics, file\_refs, source records, wikilinks\_out, wikilinks\_in, cross\_wiki\_links, and body (raw markdown). When returned by the viewer API `page()` method, a `related_pages` field is appended — page previews for all wikilinks\_in, wikilinks\_out, and supersedes/superseded\_by targets, deduplicated, for the frontend to render titles without extra fetches.
...
The jobs dashboard (`/jobs`) lists all job records in reverse-chronological order. Each row shows: operation badge, provider/model, optional transcript source for session captures, `displayStatus` badge (colored by status), `displayTitle`, `displaySubtitle` (page-change description if available), and elapsed time. Clicking any row navigates to `/jobs/:jobId`.
...
- **Stream timeline**: assistant text renders as chat bubbles; Normal mode groups root tool calls into collapsed activity groups; Debug mode renders individual tool cards; tool results are paired with their tool call by ID where possible; invalid lines render as error rows showing the raw content and parse error.

The desired Normal-mode hierarchy for a job detail page is outcome-first, not colophon-first. The first screen should answer: which model ran, whether it succeeded, elapsed time, what pages were created or modified, what input transcript/file/folder the run ingested, and whether that input can be opened for inspection. Page-change rows are the primary result object and should link to the affected pages when possible. Started and finished timestamps are useful but secondary. Provider session IDs, raw log paths, and other audit fields belong below the primary description or in Debug-oriented detail because they do not answer the normal "what happened in this run?" question. The separate barometer/colophon treatment should not remain always-visible in Normal mode when it repeats status, provider, token, and path facts already present in the description. Usage should show token count when available and a cost estimate when the run record has enough pricing data; if the estimate is not available, the UI may show an explicit "coming soon" placeholder rather than pretending token count alone is spend.
...
Hero-style mono eyebrow labels are not part of the current viewer direction. The page title and supporting description should carry the section identity without a `// LABEL` line above the heading.
