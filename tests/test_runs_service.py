...
from codealmanac.runs.ledger.streaming import RunAttachStreamer
...
def test_runs_service_records_run_and_events(
    ...
    runs_path = workspace_runs_path(repo)
    assert shown.log_path == runs_path / f"{record.run_id}.jsonl"
    ...
    assert not (repo / "almanac/runs" / f"{record.run_id}.json").exists()
...
def test_runs_service_targets_registered_wiki(
    ...
    assert (workspace_runs_path(first) / f"{record.run_id}.json").is_file()
...
def test_runs_service_worker_lock_is_exclusive_and_recovers_stale_owner(
    ...
    first.release()
    runs_path = workspace_runs_path(repo)
    assert (runs_path / "worker.lock").is_dir()
    recovered.release()
    assert not (runs_path / "worker.lock").exists()
