...
from codealmanac.runs.ledger.factory import new_run_id, new_run_record
from codealmanac.runs.ledger.io import RunLedgerIO
from codealmanac.runs.ledger.locks import RunWorkerLease, acquire_worker_lock
...
from codealmanac.runs.ledger.queries import (
    list_run_records,
    next_spec_backed_queued_run,
)
from codealmanac.runs.ledger.transitions import RunTransitionWriter
...
class RunStore:
    ...
    def queue(
        ...
        now = datetime.now(UTC)
        run_id = new_run_id(spec.kind, now)
        record = new_run_record(
            run_id,
            workspace_id,
            spec.kind,
    ...
    def mark_running(self, run_dir: Path, run_id: str) -> RunRecord:
        ...
        if record.status != RunStatus.QUEUED:
            raise ConflictError(f"run {run_id} cannot start from {record.status.value}")
        now = datetime.now(UTC)
