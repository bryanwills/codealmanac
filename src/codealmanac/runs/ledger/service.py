...
from codealmanac.runs.ledger.store import RunStore, RunWorkerLease
...
class RunLedgerService:
    ...
    def next_queued(self, request: NextQueuedRunRequest) -> QueuedRun | None:
        workspace = self.resolve_workspace(request.cwd, request.wiki)
        return self.store.next_queued(self.primary_run_dir(workspace))
    ...
    def primary_run_dir(self, workspace: Workspace) -> Path:
        if self.runs_path is None:
            return workspace.almanac_path / "runs"
        return self.runs_path / workspace.workspace_id
