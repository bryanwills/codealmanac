...
The branch has more than 280 committed rewrite commits past `dev`. The worklog records 266 production slices so far.
...
The latest slice split lifecycle workflow ownership into `src/services/lifecycle/workflows/`. Init owns init prompt context, Absorb owns direct and prepared Absorb handoff, Garden owns Garden run-mode policy, and provider resolution owns workflow-level provider parsing/resolution. The old `src/services/lifecycle/workflows.ts` catchall is gone.

Verification passed:

- `npm run lint`
- `npx vitest run test/architecture-lifecycle-provider-boundaries.test.ts test/operation-commands.test.ts test/sync.test.ts test/build-operation.test.ts test/absorb-operation.test.ts test/garden-operation.test.ts`
