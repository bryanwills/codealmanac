...
The branch has more than 280 committed rewrite commits past `dev`. The worklog records 269 production slices so far.
...
The latest slice moved setup provider-fix runtime wiring into setup command registration. `src/edges/cli/register-setup-command.ts` now supplies `runPlatformSetupProviderFixCommand`, and `src/edges/cli/setup/agent-choice.ts` only consumes the explicit runner passed through setup options.

Verification passed:

- `npm run lint`
- `npx vitest run test/architecture-setup-boundaries.test.ts test/setup.test.ts`
