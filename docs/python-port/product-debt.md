...
| Agent harness contract | partially resolved: `PageRunWorkflow` records normalized `HarnessEvent` values before validation, run logs persist structured harness payloads, and Codex now maps app-server notifications into text/tool/usage/provider-session/root-helper/done events. The active rewrite goal still needs the richer Claude SDK/event harness model before this contract is restored for both providers. |
