...
| Arbitrary custom root source-runtime ignores | Filesystem directory runtime excludes `almanac/`, `docs/almanac/`, and `.almanac/`. If users configure another root and ingest the repo directory, pass the workspace root into source runtime instead of adding more global ignore guesses. |
