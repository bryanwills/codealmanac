...
| Background jobs | mostly resolved for manual lifecycle operations: `jobs attach`/`jobs cancel` use durable run-ledger state; queued background work has `<run-id>.spec.json`, oldest spec-backed selection, a per-wiki worker lock, stale-lock recovery, in-process queue draining, detached worker spawning, hidden worker entrypoint, and explicit `ingest --background` / `garden --background`. Still missing: explicit product decision on whether any lifecycle verb should default to background, plus sync enqueue/background integration if scheduled sync should stop running foreground ingest in-process. |
