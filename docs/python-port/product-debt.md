...
| Setup terminal UX | partially resolved: slice 79 restores setup/uninstall-owned Codex and Claude instruction artifacts; slice 80 adds Rich-backed branded panels, status rows, and a next-step box; slice 86 exposes default agent and automation recommendations as typed setup plan facts. Still missing: raw-mode target selection UI and any explicit setup-owned automation install choice if that product decision is reopened. |
