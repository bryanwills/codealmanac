...
| Integrations behind service ports | ownership map drafted; filesystem, Git, GitHub, transcript, and web runtime adapters implement `SourceRuntimeAdapter`; transcript discovery adapters implement `TranscriptDiscoveryAdapter`; Claude SDK and Codex app-server implement `HarnessAdapter`; Git workspace change probe implements `WorkspaceChangeProbe`; architecture tests guard import direction, Claude SDK event-mapper boundaries, Codex app-server event boundaries, and Codex app-server client/policy boundaries | sources, source runtime, transcript discovery, harness, Claude/Codex adapter, Git probe, ingest safety, sync status, sync run, filesystem diversity tests, and architecture tests; real Claude and Codex ingest dogfood; fake Codex app-server protocol tests; fake Claude SDK stream tests; real filesystem/Git/GitHub/transcript/web runtime dogfood; source-runtime diversity dogfood; slice 70 non-toy source-shape dogfood; slice 85 focused Claude architecture tests; slice 100 and 101 focused Codex architecture tests | Source runtime MVP is covered; remaining integration risk is live-provider dogfood for the richer harness transports. |
...
| Gate | Command | 2026-07-01 result |
|---|---|---|
| Focused CLI, automation, setup, and architecture tests | `uv run pytest tests/test_cli.py tests/test_automation_service.py tests/test_setup_service.py tests/test_architecture.py::test_cli_has_separate_parser_dispatch_and_render_packages tests/test_architecture.py::test_cli_admin_dispatch_stays_split_by_command_family tests/test_architecture.py::test_cli_dispatch_files_stay_small -q` | 56 passed |
| Focused lint | `uv run ruff check src/codealmanac/cli tests/test_cli.py tests/test_automation_service.py tests/test_setup_service.py tests/test_architecture.py` | passed |
| Isolated admin CLI dogfood | temp `HOME`; temp repo; `init`; service-created run event; `doctor --json`; `automation status --json`; `setup --json`; `uninstall --json`; `update --check --json`; `jobs --json`; `jobs show/logs/attach --json` | passed; each moved admin dispatcher was exercised through public CLI commands |
| Full tests | `uv run pytest` | 343 passed |
| Full lint | `uv run ruff check .` | passed |
| Diff hygiene | `git diff --check` | passed |

## Gates For Slice 123 CLI Admin Render Boundaries

| Gate | Command | 2026-07-01 result |
|---|---|---|
| Focused CLI, automation, setup, and architecture tests | `uv run pytest tests/test_cli.py tests/test_automation_service.py tests/test_setup_service.py tests/test_architecture.py::test_cli_has_separate_parser_dispatch_and_render_packages tests/test_architecture.py::test_cli_render_root_stays_facade tests/test_architecture.py::test_cli_admin_render_stays_split_by_output_family -q` | 56 passed |
| Focused lint | `uv run ruff check src/codealmanac/cli tests/test_cli.py tests/test_automation_service.py tests/test_setup_service.py tests/test_architecture.py` | passed |
| Isolated admin CLI dogfood | temp `HOME`; temp repo; `init`; service-created run event; `doctor --json`; `automation status --json`; `setup --json`; `uninstall --json`; `update --check --json`; `jobs --json`; `jobs show/logs/attach --json` | passed; each moved admin renderer was exercised through public CLI commands |
| Full tests | `uv run pytest` | 342 passed |
| Full lint | `uv run ruff check .` | passed |
| Diff hygiene | `git diff --check` | passed |
...
| Gate | Command | 2026-07-01 result |
|---|---|---|
| Focused CLI and architecture tests | `uv run pytest tests/test_cli.py tests/test_topics_mutation.py tests/test_architecture.py::test_cli_has_separate_parser_dispatch_and_render_packages tests/test_architecture.py::test_cli_dispatch_edge_is_split_by_command_domain tests/test_architecture.py::test_cli_dispatch_files_stay_small -q` | 52 passed |
| Focused lint | `uv run ruff check src/codealmanac/cli tests/test_cli.py tests/test_topics_mutation.py tests/test_architecture.py` | passed |
| Isolated public CLI dogfood | temp `HOME`; temp repo; `init`; `list --json`; `search`; `show --lead`; `topics create/show/delete`; live `serve` API smoke | passed; list/search/show/topic commands worked and live serve overview returned the temp wiki |
...
| Gate | Command | 2026-07-01 result |
|---|---|---|
| Focused behavior and architecture tests | `uv run pytest tests/test_topics_mutation.py tests/test_topics_health.py tests/test_read_model.py tests/test_architecture.py::test_wiki_topics_yaml_stays_split_by_read_and_mutation -q` | 27 passed |
| Focused lint | `uv run ruff check src/codealmanac/services/wiki src/codealmanac/services/topics tests/test_topics_mutation.py tests/test_topics_health.py tests/test_read_model.py tests/test_architecture.py` | passed |
| Isolated topic command dogfood | temp `HOME`; temp repo; `codealmanac init`; `topics create/link/show/rename/delete`; `health --json` | passed; strict shell run verified parent links, rename slug, delete cleanup, and empty-topic health cleanup |
| Full tests | `uv run pytest` | 340 passed |
| Full lint | `uv run ruff check .` | passed |
| Diff hygiene | `git diff --check` | passed |

## Gates For Slice 118 Server Route Boundaries

| Gate | Command | 2026-07-01 result |
|---|---|---|
| Focused behavior and architecture tests | `uv run pytest tests/test_server.py tests/test_architecture.py::test_server_app_stays_composition_root tests/test_architecture.py::test_viewer_jobs_surface_stays_read_only -q` | 10 passed |
| Focused lint | `uv run ruff check src/codealmanac/server tests/test_server.py tests/test_architecture.py` | passed |
| Live serve API dogfood | temp `HOME`; temp repo; current-checkout `codealmanac serve`; `curl /`, `/api/overview`, `/api/search?q=getting`, `/assets/viewer/main.js` | passed; initialized temp wiki, served shell, overview, search result, and nested viewer module |
| Full tests | `uv run pytest` | 339 passed |
| Full lint | `uv run ruff check .` | passed |
| Diff hygiene | `git diff --check` | passed |

## Gates For Slice 117 Automation Service Boundaries

| Gate | Command | 2026-07-01 result |
|---|---|---|
| Focused lint | `uv run ruff check src/codealmanac/services/automation tests/test_automation_service.py tests/test_architecture.py` | passed |
| Focused behavior and architecture tests | `uv run pytest tests/test_automation_service.py tests/test_architecture.py::test_automation_service_keeps_selection_and_job_construction_boundaries` | 8 passed |
| Isolated service dogfood | temp app with fake scheduler installs/statuses/uninstalls sync/Garden automation | passed; verified sync/Garden install, status, uninstall, `--quiet 0s`, and normalized Garden working directory |
| Full tests | `uv run pytest` | 338 passed |
| Full lint | `uv run ruff check .` | passed |
| Diff hygiene | `git diff --check` | passed |
...
| Gate | Command | 2026-07-01 result |
|---|---|---|
| Focused lint | `uv run ruff check src/codealmanac/workflows/sync tests/test_architecture.py` | passed |
| Focused behavior and architecture tests | `uv run pytest tests/test_sync_workflow.py tests/test_architecture.py` | 40 passed |
| Isolated CLI smoke | temp `HOME` with `uv run codealmanac sync status --from codex --quiet 0s --json` | passed; returned zero scanned/eligible transcripts |
| Full tests | `uv run pytest` | 313 passed |
| Full lint | `uv run ruff check .` | passed |
| Diff hygiene | `git diff --check` | passed |
...
## Gates For Slice 101 Codex App-Server Client Boundaries

| Gate | Command | 2026-07-01 result |
|---|---|---|
| Focused Codex app-server/client tests | `uv run pytest tests/test_codex_app_server_adapter.py tests/test_codex_adapter.py tests/test_architecture.py` | passed; 36 tests |
| Focused Codex lint | `uv run ruff check src/codealmanac/integrations/harnesses/codex tests/test_codex_app_server_adapter.py tests/test_codex_adapter.py tests/test_architecture.py` | passed |
| Fake app-server client dogfood | temporary fake Codex executable used through `CodexAppServerClient` | passed; produced provider-session, tool, text, usage, and done events |
| Full tests | `uv run pytest` | passed; 324 tests |
| Full lint | `uv run ruff check .` | passed |
| Diff hygiene | `git diff --check` | passed |
