...
The workflows then compose services around the shared operation runner. `IngestWorkflow` receives sources, ingest operations, and manuals. `GardenWorkflow` receives index, health, garden operations, and manuals. `BuildWorkflow` receives repositories, wiki, build operations, and manuals [@app-root]. Packaged Yoke agents are loaded at the harness integration edge rather than injected as a prompt-rendering service. This is the bridge to the broader [Service Boundaries](service-boundaries) rule: workflows coordinate services, but the composition root provides the machinery.
